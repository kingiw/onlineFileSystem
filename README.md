...
上述的简易文件系统的功能设计中，核心在于三个部分，用户（User）、文件夹（Directory）以及文件（File），而整个文件系统我们决定采用树状结构进行管理。
分析三个部分之间的关系可以发现：

* 用户与文件夹之间受到了权限的约束。
* 文件夹之间由文件系统自身的层次结构约束。
* 文件与文件夹之间受到了从属关系的约束。
* 用户和文件之间在我们的设计中并不存在约束（因为权限控制中是以文件夹为单位的）。

作为补充，我们希望能够记录下用户对整个文件系统（即文件夹与文件）的所有行为以方便管理员进行监控，而这些日志不应因为用户、文件夹或文件发生变化而失效。
综合上述分析，我们得到了如下的E-R图：
...
一般情况下我们可以使用邻接表来存储一个树状结构，也就是保存有点以及其子节点的邻接关系的表。但是这种形式下，为了确定一个树状结构索引对应的节点，由于邻接表只存储当前节点以及其直接邻接的子节点。因此在查询子节点的邻接表前，我们是无法得知第二代子节点的。因此对于一个深度为$h$的节点的索引，一共需要进行$h$次查询，每次只为了获得某一个节点的邻接关系，对于一棵含有$n$个节点的树，其对应的平均查询次数为$O(log n)$。这样的查询方式效率是很低的，我们并没有采纳这种做法。
...
Express提供了对SQL的读写库，项目中所有对数据库的读写都是使用了它。

### 前端与后端的搭建

我们的网站采用的Nodejs(Express)+SQL的框架，前端UI使用Semantic-UI，使用nginx做了反向代理，后端和数据库的交互使用Sequelize ORM来进行。由于与网站相关的技术与本门课程没有太多关联，在这里我们仅简单对我们所使用的一些工具和基本方法做介绍。
...
[Sequelize](http://docs.sequelizejs.com/)是一个ORM库，它可以将nodejs后端的对象和SQL命令参数、返回值之间实现相互转换。并提供了简单的异常处理和数据校验。由于实际的后端中为了处理数据库中的简单数据类型和前端需要的json格式之间转换十分复杂，使用ORM可以简化这个过程。
因此在实际实现中，我们使用Sequelize来负责和数据库的直接交互，并在这个基础上将校验和处理过程封装为后端数据库的接口。详细实现可以参照`dbi.js`中的代码。
...
在一开始我们是考虑使用MongoDB，主要是因为MongoDB是一个轻量级非关系型数据库，对存储数据的模型不会像关系型数据库那样死板，最能体现这一点的就是在“存树结构”上。然而鉴于本课介绍的是关系型数据库，我们最终还是选择把MySQL作为我们项目的数据库。
...
[Getting Started with Node, Express and Mysql Using Sequelize](https://medium.com/@prajramesh93/getting-started-with-node-express-and-mysql-using-sequelize-ed1225afc3e0)
